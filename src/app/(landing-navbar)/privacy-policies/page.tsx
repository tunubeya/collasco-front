...
export async function generateMetadata(): Promise<Metadata> {
  const t = await getTranslations('privacy-policies');
  return {
    title: t('title'),
    description: t('policies.0.description'),
  };
}
...
export default function PrivacyPoliciesPage() {
  ...
  const policies = Array.from({ length: 10 }, (_, i) => ({
    id: `${i + 1}`,
    title: t(`policies.${i}.title`),
    description: t(`policies.${i}.description`),
  }));
    ...
    <div className="min-h-[calc(100vh-64px)] w-full 
      ...
      <div className="w-full max-w-4xl bg-surface 
        ...
        <div className="max-h-[70vh] overflow-y-auto space-y-6 divide-y divide-[color:var(--color-border)] pr-2">
          {policies.map((policy) => (
            <div key={policy.id} className="pt-4 first:pt-0">
